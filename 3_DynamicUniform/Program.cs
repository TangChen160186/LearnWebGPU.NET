...
namespace _3_DynamicUniform
{
    ...
    internal unsafe class Program
    {
        ...
        private static ulong _dynamicUniformOffset;
        private static uint _uniformStride;
        ...
        private static void WindowRender(double obj)
        {
            ...
            renderTime += (float)obj;
            // uniform first
            UniformData data = new UniformData()
                { Color = new Vector3((float)Math.Sin(renderTime), (float)Math.Cos(renderTime), 1) ,Time = renderTime};
            _wgpu.QueueWriteBuffer(_queue,_uniformBuffer,0, data,(UIntPtr)sizeof(UniformData));


            // uniform second
            data = new UniformData()
                { Color = new Vector3((float)Math.Cos(renderTime), (float)Math.Sin(renderTime), 1), Time = renderTime - 10};
            _wgpu.QueueWriteBuffer(_queue, _uniformBuffer, _uniformStride, data, (UIntPtr)sizeof(UniformData));
            ...
            _wgpu.RenderPassEncoderSetIndexBuffer(renderPass,_indexBuffer,IndexFormat.Uint16,0,_wgpu.BufferGetSize(_indexBuffer));

            // draw 1
            _wgpu.RenderPassEncoderSetBindGroup(renderPass, 0, _bindGroup, 1, 0);
            _wgpu.RenderPassEncoderDrawIndexed(renderPass,(uint)_indexData.Length,1,0,0,0);
            // draw2
            _wgpu.RenderPassEncoderSetBindGroup(renderPass, 0, _bindGroup, 1, _uniformStride);
            _wgpu.RenderPassEncoderDrawIndexed(renderPass, (uint)_indexData.Length, 1, 0, 0, 0);
        ...
        private static void InspectDevice()
        {
            ...
            _dynamicUniformOffset = supportedLimits.Limits.MinUniformBufferOffsetAlignment;
            // get device features
        ...
        private static void InitializePipeline()
        {
            ...
            VertexAttribute* attributes = stackalloc VertexAttribute[] { positionAttribute};
            ...
            BufferBindingLayout bufferBindingLayout = new BufferBindingLayout()
            {
                ...
                // change HasDynamicOffset to true
        ...
        private static void InitializeBuffers()
        {
            ...
            // uniform buffer
            var uniformDataSize = sizeof(UniformData);
            _uniformStride = CeilToNextMultiple((uint)uniformDataSize, (uint)_dynamicUniformOffset);

            BufferDescriptor uniformBufferDescriptor = new BufferDescriptor()
            {
                MappedAtCreation = false,
                Size = _uniformStride + (uint)(uniformDataSize),
        ...
        {
            uint divideAndCeil = (uint)(value / step + (ulong)(value % step == 0 ? 0 : 1));
