...
namespace _1_FirstTexture
{
    ...
    internal unsafe class Program
    {
        ...
        private static Texture* _baseTexture;
        private static TextureView* _baseTextureView;
        ...
        private static void WindowRender(double obj)
        {
            ...
            _modelMat *= Matrix4x4.Identity;
            data.Model = _modelMat;
            data.View = Matrix4x4.CreateLookAt(new Vector3(0,0,1f),Vector3.Zero, Vector3.UnitY);
        ...
        private static void InitializePipeline()
        {
            ...
            VertexAttribute uvAttribute = new VertexAttribute()
            {
                Format = VertexFormat.Float32x2,
                Offset = sizeof(float) * 3,
                ShaderLocation = 1,
            };
            VertexAttribute* attributes = stackalloc VertexAttribute[] { positionAttribute, uvAttribute };
            ...
            BindGroupLayoutEntry bindGroupLayoutEntry1 = new BindGroupLayoutEntry
            {
                Binding = 1,
            ...
            TextureBindingLayout textureBindingLayout = new TextureBindingLayout()
            {
                Multisampled = false,
                SampleType = TextureSampleType.Float,
                ViewDimension = TextureViewDimension.Dimension2D,
            };
            BindGroupLayoutEntry bindGroupLayoutEntry2 = new BindGroupLayoutEntry
            {
                Binding = 2,
                Visibility = ShaderStage.Vertex | ShaderStage.Fragment,
                Texture = textureBindingLayout,
            };
            BindGroupLayoutEntry* bindGroupEntries = stackalloc BindGroupLayoutEntry[]
                { bindGroupLayoutEntry, bindGroupLayoutEntry1, bindGroupLayoutEntry2 };
        ...
        private static void BindingResources()
        {
            ...
            Sampler* _sampler = _wgpu.DeviceCreateSampler(_device, samplerDescriptor);
            ...
            BindGroupEntry bindGroupEntry1 = new BindGroupEntry()
            {
                Binding = 1,
                Sampler = _sampler,
            };
            BindGroupEntry bindGroupEntry2 = new BindGroupEntry()
            {
                Binding = 2,
                TextureView = _baseTextureView,
            };
            BindGroupEntry* entries = stackalloc BindGroupEntry[] { bindGroupEntry, bindGroupEntry1, bindGroupEntry2 };
        ...
        private static void CreateBindingTextures()
        {
            ...
            _baseTexture = _wgpu.DeviceCreateTexture(_device, textureDescriptor);
            ...
            _baseTextureView = _wgpu.TextureCreateView(_baseTexture, textureViewDescriptor);

            byte* pixels = stackalloc byte[(int)(textureDescriptor.Size.Width * textureDescriptor.Size.Height * 4)];
            for (int i = 0; i < textureDescriptor.Size.Width; ++i)
            {
                for (int j = 0; j < textureDescriptor.Size.Height; ++j)
                {
                    byte* p = &pixels[4 * (j * textureDescriptor.Size.Width + i)];
                    p[0] = (byte)i; // r
                    p[1] = (byte)j; // g
                    p[2] = 128; // b
                    p[3] = 255; // a
            ...
            _wgpu.QueueWriteTexture(_queue,&destination,pixels, textureDescriptor.Size.Width * textureDescriptor.Size.Height * 4,dataLayout, textureDescriptor.Size);
